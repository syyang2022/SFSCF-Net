...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=384, patch_size=4, in_chans=3, num_classes=200,
...
class FDM(nn.Module):
    ...
    def forward(self, fm1, fm2):
        # torch.Size([2, 256, 96, 96])  torch.Size([2, 256, 48, 48])

        # b, c, w1, h1 = fm1.shape
        _, _, w, h = fm2.shape
        fm1 = F.interpolate(fm1, size=(w, h), mode='bilinear', align_corners=True)

        ac = torch.mul(fm1, fm2)
        ac = self.channel_attention(ac)
        newfm = torch.mul(ac, fm2) + fm2

        return newfm

class SwinNet(nn.Module):
    def __init__(self, feature_size=512, class_num=200):
        ...
        self.up2 = nn.UpsamplingBilinear2d(scale_factor=1)
        self.up4 = nn.UpsamplingBilinear2d(scale_factor=4)

        self.conv2048_1024 = conv3x3_bn_relu(1024, 2048)
        self.conv1024_512 = conv3x3_bn_relu(512, 1024)
        self.conv512_256 = conv3x3_bn_relu(256, 512)
        self.conv256_128 = conv3x3_bn_relu(128, 256)
        ...
        self.max1 = nn.MaxPool2d(kernel_size=12, stride=12)
        self.max2 = nn.MaxPool2d(kernel_size=24, stride=24)
        self.max3 = nn.MaxPool2d(kernel_size=48, stride=48)

        self.num_ftrs = 2048 * 1 * 1

        self.inter1 = FDM(256)
        self.inter2 = FDM(512)
        self.inter3 = FDM(1024)

        self.drop = nn.Dropout(p=0.5)
        self.cl1 = nn.Linear(1024, class_num)
        self.cl2 = nn.Linear(512, class_num)
        self.cl3 = nn.Linear(256, class_num)
        self.cl4 = nn.Linear(128, class_num)

    def forward(self, x, d):
        # print(x.shape, d.shape) torch.Size([2, 3, 384, 384])
        ...
        # torch.Size([2, 1024, 12, 12]) torch.Size([2, 512, 24, 24])
        # torch.Size([2, 256, 48, 48]) torch.Size([2, 128, 96, 96])
        #

        rc1 = self.conv256_128(r1)
        fr2_r1 = self.inter1(rc1, r2)
        # print(rc1.shape, r2.shape) torch.Size([2, 256, 96, 96]) torch.Size([2, 256, 48, 48])
        rc2 = self.conv512_256(fr2_r1)
        fr3_r12 = self.inter2(rc2, r3)
        rc3 = self.conv1024_512(fr3_r12)
        fr4_r123 = self.inter3(rc3, r4)
        # fr4_r1, fr1_r4 = self.inter(r4, r1)

        dc1 = self.conv256_128(d1)
        fd2_d1 = self.inter1(dc1, d2)
        dc2 = self.conv512_256(fd2_d1)
        fd3_d12 = self.inter2(dc2, d3)
        dc3 = self.conv1024_512(fd3_d12)
        fd4_d123 = self.inter3(dc3, d4)
        # print(fr4_r123.shape, fd4_d123.shape)
        fm1 = r4 + fr4_r123
        fm2 = d4 + fd4_d123

        x1 = F.adaptive_avg_pool2d(r4, (1, 1))
        x1 = x1.flatten(1)
        y1 = F.adaptive_avg_pool2d(d4, (1, 1))
        y1 = y1.flatten(1)
        # print(x1.shape)
        ...
        features1_self = torch.mul(fr1, r4) + r4
        features1_other = torch.mul(fd1, r4) + r4

        features2_self = torch.mul(fd1, d4) + d4
        features2_other = torch.mul(fr1, d4) + d4
